...
namespace NNChallenge.iOS.ViewModel
{
    public class ForecastDataSource : UITableViewSource
    {
        ...
        {
            var cell = tableView.DequeueReusableCell("HourViewCell", indexPath) as HourViewCell;
            ...
            var weather = string.Format("{0}ºC / {1}ºF", temperatureC, temperatureF);
            var datetime = _hourForecast[indexPath.Row].Date;
            cell.Bind(weather, imgUrl, datetime);
